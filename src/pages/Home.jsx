...
import {
  ...
  Zap, MapPin, Clock, Flame, Star, ArrowRight, TrendingUp, Shield,
} from "lucide-react";
...
function Particles() {
  ...
  return (
    <div className="absolute inset-0 pointer-events-none overflow-hidden">
      {particles.map((p) => (
        <div key={p.id} className="particle-dot"
          style={{ left:`${p.x}%`, top:`${p.y}%`, width:p.size, height:p.size,
            opacity:p.opacity, animationDelay:`${p.delay}s`, animationDuration:`${p.duration}s` }}/>
      ))}
      {[0,1,2,3,4].map((i) => (
        <div key={`ball-${i}`} className="particle-ball"
          style={{ left:`${10+i*18}%`, animationDelay:`${i*2.2}s`,
            animationDuration:`${13+i*2.5}s`, fontSize:`${13+i*3}px` }}>⚽</div>
      ))}
    </div>
  );
}
...
function TodaySection({ navigate }) {
  ...
  const todayMatches = useMemo(
    () => sortByDateTime(fixtures.filter((f) => f.date === today)), [today]);
        ...
        <div className="pre-tournament-banner">
          ...
          <div className="relative z-10 text-center py-12 px-6">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full mb-6 text-xs font-bold tracking-widest uppercase"
              style={{ background:"rgba(0,216,76,0.08)", border:"1px solid rgba(0,216,76,0.3)", color:"#00D84C" }}>
            ...
            <p className="text-gray-400 text-sm mb-10">
              Opening match: <span style={{color:"#00D84C"}}>12 June 2026</span> — Estadio Azteca, Mexico City
            </p>
            {countdown && (
            ...
            <p className="mt-10 text-xs text-gray-600">সময় Bangladesh Standard Time (UTC+6) অনুযায়ী</p>
          </div>
        </div>
      </section>
    );
      ...
      <section className="max-w-6xl mx-auto px-4 pb-16 animate-section">
        <div className="finished-banner">
          <div className="text-center py-14 px-6 relative z-10">
            ...
            <h2 className="text-4xl text-white mb-3 tracking-wide">Tournament Complete!</h2>
            <p className="text-gray-400 text-sm mb-8">FIFA World Cup 2026 শেষ হয়েছে। সকল ফিক্সচার আর্কাইভ দেখুন।</p>
            <button onClick={()=>navigate("/by-date")} className="btn-gold px-8 py-3 text-sm rounded-xl inline-flex items-center gap-2">
    ...
    <section className="max-w-6xl mx-auto px-4 pb-16 animate-section">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl text-white tracking-wide leading-none">Match Center</h2>
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={()=>navigate("/by-date")}
      ...
      {activeTab === "today" ? (
        todayMatches.length === 0 ? (
          <div className="no-matches-today">
            ...
            <p className="text-xl text-white tracking-wide mb-1">আজ কোনো ম্যাচ নেই</p>
            <p className="text-gray-500 text-sm">No matches scheduled for today.</p>
            <button onClick={()=>setActiveTab("upcoming")} className="mt-5 btn-gold px-6 py-2.5 text-sm rounded-xl inline-flex items-center gap-1.5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {todayMatches.map((match, i) => (
              <div key={match.id} className="card-stagger" style={{animationDelay:`${i*80}ms`}}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {upcomingMatches.length === 0 ? (
            <div className="col-span-full no-matches-today">
              <p className="text-gray-400 text-sm">No upcoming matches found.</p>
            </div>
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
      {GROUPS.map((g, gi) => {
        ...
        return (
          <div key={g} className="group-card card-stagger" style={{animationDelay:`${gi*50}ms`}}>
    ...
    <div className="featured-match px-6 py-5 flex flex-col md:flex-row items-center justify-between gap-4 mb-8">
      ...
      <div className="flex items-center gap-5">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-xl text-white tracking-wider">{next.team1}</span>
        </div>
        <span className="text-xs font-bold px-3 py-1.5 rounded-lg text-green-400"
          style={{background:"rgba(0,216,76,0.08)",border:"1px solid rgba(0,216,76,0.2)"}}>VS</span>
        <div className="flex items-center gap-2.5">
          <span className="text-xl text-white tracking-wider">{next.team2}</span>
    ...
    <div className="hero-bg pitch-lines min-h-screen">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative max-w-6xl mx-auto px-4 pt-20 pb-16 text-center"
          ...
          {/* Trophy */}
          <div className="trophy-icon animate-fade-down" style={{animationDelay:"0.1s"}}>🏆</div>

          {/* Heading */}
          <h1 className="text-5xl md:text-7xl leading-none mb-1 animate-fade-up gold-text tracking-wide"
            style={{animationDelay:"0.15s"}}>
            FIFA World Cup
          </h1>
          <h2 className="text-8xl md:text-[10rem] text-white mb-4 animate-fade-up year-text leading-none"
          ...
          <div className="animate-fade-up" style={{animationDelay:"0.25s"}}>
            <div className="host-strip">
              ...
              <span className="text-sm font-semibold text-gray-300">🇨🇦 Canada</span>
              <div className="host-sep"/>
              <span className="text-sm font-semibold text-gray-300">🇲🇽 Mexico</span>
            </div>
          </div>

          <p className="text-sm text-gray-500 mt-3 mb-10 animate-fade-up" style={{animationDelay:"0.3s"}}>
            সব সময়{" "}
            <span style={{color:"#00D84C"}}>Bangladesh Standard Time (UTC+6)</span> অনুযায়ী
          </p>

          {/* Search */}
          <form onSubmit={handleSearch}
            className="max-w-xl mx-auto relative animate-fade-up" style={{animationDelay:"0.35s"}}>
            <div className="search-wrapper">
              <Search size={18} className="search-icon" style={{color:"#00D84C"}}/>
              <input type="text" className="field-input search-input"
                placeholder="দল খুঁজুন (যেমন Brazil, Germany…)"
                value={search} onChange={(e) => setSearch(e.target.value)}/>
              <button type="submit" className="btn-gold search-btn">খুঁজুন</button>
            </div>
          </form>
        </div>
      </section>
      ...
      <section className="max-w-6xl mx-auto px-4 pb-14 animate-section">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { icon:CalendarDays, label:"Total Matches",       value:fixtures.length, color:"#00D84C"  },
            { icon:Users,        label:"Participating Teams", value:TEAM_COUNT,      color:"#39FF8A"  },
            { icon:Trophy,       label:"Groups",              value:GROUPS.length,   color:"#00A83A"  },
            { icon:Globe,        label:"Host Countries",      value:3,               color:"#A0FFD0"  },
      ...
      <section className="max-w-6xl mx-auto px-4 pb-16 animate-section">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[
            {
              path:"/by-team", icon:Users,
              title:"Fixtures by Team",
              desc:`All ${TEAM_COUNT} teams — select any to view their full tournament schedule`,
              iconBg:"rgba(0,168,58,0.15)", iconBorder:"rgba(0,216,76,0.3)", iconColor:"#00D84C",
              badge:"48 Teams",
            },
            {
              path:"/by-date", icon:CalendarDays,
              title:"Fixtures by Date",
              desc:"Browse all matches day-by-day — Group Stage through the Final",
              iconBg:"rgba(0,100,40,0.15)", iconBorder:"rgba(0,216,76,0.25)", iconColor:"#39FF8A",
              badge:"Jun–Jul",
            },
          ].map((item) => (
            <button key={item.path} type="button"
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2.5 mb-1">
                  <span className="text-xl text-white tracking-wide">{item.title}</span>
                  <span className="text-xs font-bold px-2 py-0.5 rounded-full"
                    style={{background:"rgba(0,216,76,0.1)",color:"#00D84C",border:"1px solid rgba(0,216,76,0.2)"}}>
                    {item.badge}
                  </span>
                </div>
                <div className="text-sm text-gray-400">{item.desc}</div>
              </div>
              <ChevronRight size={20} className="flex-shrink-0 text-gray-600 nav-arrow"/>
            </button>
          ))}
        </div>
      </section>
        ...
        {[
          ...
          <div key={section.country} className="mb-8">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-base tracking-wider text-gray-300">{section.country}</span>
              <div className="flex-1 h-px" style={{background:"rgba(0,216,76,0.1)"}}/>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
              {section.venues.map((v, i) => (
                <div key={v.name} className="venue-card card-stagger"
                  style={{animationDelay:`${(si * section.venues.length + i)*35}ms`}}>
